...
#define slomt _p[4]
#define taummin _p[5]
#define taumdiff1 _p[6]
#define taumdiff2 _p[7]
#define offh _p[8]
#define sloh _p[9]
#define offht1 _p[10]
#define offht2 _p[11]
#define sloht _p[12]
#define tauhmean _p[13]
#define tauhdiff1 _p[14]
#define tauhdiff2 _p[15]
#define ik _p[16]
#define gK_Pst _p[17]
#define m _p[18]
#define h _p[19]
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 K_Pst /home/christoph/MMN_project/Capstone/x86_64/K_Pst.mod\n");
...
static int  rates ( _threadargsproto_ ) {
   double _lqt , _lthresh ;
 _lqt = pow( 2.3 , ( ( 34.0 - 21.0 ) / 10.0 ) ) ;
   _lthresh = offmt - slomt / 2.0 * log ( taumdiff1 / taumdiff2 ) ;
    mInf = ( 1.0 / ( 1.0 + exp ( ( offm - v ) / slom ) ) ) ;
   if ( v < _lthresh ) {
     mTau = ( taummin + taumdiff1 * exp ( - ( offmt - v ) / slomt ) ) / _lqt ;
     }
   else {
     mTau = ( ( taummin + taumdiff2 * exp ( ( offmt - v ) / slomt ) ) ) / _lqt ;
     }
   hInf = 1.0 / ( 1.0 + exp ( - ( offh - v ) / sloh ) ) ;
   hTau = ( tauhmean + ( tauhdiff1 - tauhdiff2 * ( offht1 - v ) ) * exp ( - pow( ( ( offht2 - v ) / sloht ) , 2.0 ) ) ) / _lqt ;
     return 0; }
