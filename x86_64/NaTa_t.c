...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 NaTa_t /home/christoph/MMN_project/Capstone/x86_64/NaTa_t.mod\n");
...
static int  rates ( _threadargsproto_ ) {
 ...
 _lqt = pow( 2.3 , ( ( 34.0 - 21.0 ) / 10.0 ) ) ;
   ...
   mAlpha = - ( offm - v ) / ( 1.0 - ( exp ( ( offm - v ) / slom ) ) ) / tauma ;
   mBeta = ( offm - v ) / ( 1.0 - ( exp ( - ( offm - v ) / slom ) ) ) / taumb ;
   mTau = ( 1.0 / ( mAlpha + mBeta ) ) / _lqt ;
   mInf = mAlpha / ( mAlpha + mBeta ) ;
   if ( v  == offh ) {
     v = v + 0.0001 ;
     }
   hAlpha = ( offh - v ) / ( 1.0 - ( exp ( - ( offh - v ) / sloh ) ) ) / tauha ;
   hBeta = - ( offh - v ) / ( 1.0 - ( exp ( ( offh - v ) / sloh ) ) ) / tauhb ;
