...
#define tauha _p[16]
#define tauhb _p[17]
#define tauhmax _p[18]
#define ina _p[19]
#define gNap_Et2 _p[20]
#define m _p[21]
#define h _p[22]
#define ena _p[23]
#define mInf _p[24]
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 Nap_Et2 /home/christoph/MMN_project/Capstone/x86_64/Nap_Et2.mod\n");
...
static int  rates ( _threadargsproto_ ) {
 ...
 _lqt = pow( 2.3 , ( ( 34.0 - 21.0 ) / 10.0 ) ) ;
   ...
   mAlpha = - ( offma - v ) / ( 1.0 - ( exp ( ( offma - v ) / sloma ) ) ) / tauma ;
   mBeta = ( offmb - v ) / ( 1.0 - ( exp ( - ( offmb - v ) / slomb ) ) ) / taumb ;
   mTau = taummax * ( 1.0 / ( mAlpha + mBeta ) ) / _lqt ;
   if ( v  == offha ) {
     v = v + 0.0001 ;
     }
   if ( v  == offhb ) {
   ...
   hAlpha = ( offha - v ) / ( 1.0 - exp ( - ( offha - v ) / sloha ) ) / tauha ;
   hBeta = - ( offhb - v ) / ( 1.0 - exp ( ( offhb - v ) / slohb ) ) / tauhb ;
   hTau = tauhmax * ( 1.0 / ( hAlpha + hBeta ) ) / _lqt ;
     return 0; }
